...
export function useDeduplication() {
  ...
  const results = ref<DuplicateResultType>({
    duplicate_groups: [],
    stats: {duplicate_groups: 0, total_items: 0, unique_items: 0},
  });
  ...
  watch(strategy, async (newStrategy, oldStrategy) => {
    if (isInternalUpdate.value) {
      isInternalUpdate.value = false; // Reset flag
      return; // Skip watcher execution
    }
    if (newStrategy && JSON.stringify(newStrategy) !== JSON.stringify(oldStrategy)) {
      try {
        ...
        updateStrategyFromServer(updatedStrategy);
